...
banner:
  content: |
    次回の販売開始まで あと<span class="countdown"><countdown-timer data-target="2026-04-12T12:00:00+09:00"></countdown-timer></span>
hero:
  ...
  actions:
    ...
    - text: ZaruBall v3 Build Guide
      link: /ZaruBallv3-docs/build-guide/preparation/
      icon: document
      variant: secondary
...
---
    ...
    {/* 在庫は完売いたしました。次回頒布については X(Twitter) のポストをお待ちください！ */}
...
<CardGrid>
  <CustomCard title="充電機能">
    <Charge slot="icon" class="theme-icon" font-size="1.5rem" aria-hidden />
...
現在、在庫、予約分共に**売り切れ**です。
次回のは4/7 12:00に10台分入荷予定です。X上での告知をお待ちください。

:::

<div style={{ display: 'flex', justifyContent: 'center' }}>

| 名称                               | 金額     | 在庫数(4/7 12:00～) | 予約注文残数 |
| ---------------------------------- | -------- | ------------------- | ------------ |
...
- **同梱しない部品**：トラックボール、スイッチ、キーキャップ、バッテリー、トラックボールは付属しません。ご自身でご購入下さい。
